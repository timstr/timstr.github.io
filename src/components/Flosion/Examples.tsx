    ...
    <>
        ...
        <Section header={<h2>Basic Setups</h2>}>
            <p>
                ...
                through a "Sine" object, and into a graph. Graphs were a very useful feature that
                unfortunately were implemented in a very unsafe and messy way, and ways to
                reintroduce them are under investigation. At the bottom of the image, there is the
                object labeled "Aaah.wav," which is simply a sound object that plays a sound file.
                This object is fed into the "PlayBack" object, which simply takes the sound stream
                entering its input and plays the stream to the computer's speakers. The result is
                that one can physically here myself screaming.
            </p>
            <p>
                If the font looks weird, it's probably because the glyphs were hand drawn by myself,
                using a drawing program that I also wrote myself to animate video game characters,
            ...
            <p>
                ...
                spectrogram can be used to see how the sound is composed of a distribution of waves
                of different frequencies and amplitudes. The Filter object can be used to amplify or
                attenuate these frequencies selectively. It exposes a "frequency" number source at
                its top, which it can change to select a frequency, and it provides an "amplitude"
                number input, which it multiplies individual frequencies by. To modify individual
                frequencies, one simply needs to define a function of the Filter's frequency and
                pass it into the "amplitude" input. In this screenshot, a bell curve, or Gaussian
                distribution, is used to silence all but a narrow range of frequency components. The
                overall result is that a{" "}
                <Link dst="https://en.wikipedia.org/wiki/Band-pass_filter">band-pass filter</Link>{" "}
                is applied to the sound stream. Band-pass filters are a fairly common effect in
                digital music production, and can be used for things like isolating vocals or bass
                from a larger piece of audio.
            </p>
            ...
            <p>
                ...
                , which produces some periodic signal at a specified frequency. The NoteGen object
                exposes a "phase" number output, visible on its top edge, and accepts a "wave
                function" input. By defining the wave function input to be any desirable function
                the phase, the user can create any wave form they choose. The wave function chose
                here is the canonical sine wave.
            </p>
            <p>
                The frequency at which this wave form is played in the output sound is determined by
                the "frequency" number input, which is being given the constant value of 250 Hz
                here. The end result of this setup is a simple sine-wave tone of 250 Hz.
            </p>
            ...
            <p>
                ...
                horizontal axis denotes time. Essentially, it represents sheet music, but without
                all the swirls and lines and other complex rules. The Sampler uses a multi-input so
                that it can call upon its input many times in parallel, which it needs to do when
                its notes overlap in time. This example thus plays a sine-wave melody which is
                defined by the notes drawn on the Sampler object.
            </p>
        ...
        <Section header={<h2>More Graphs</h2>}>
            <p>
                In previous versions where graphs still existed, 2D graphs could also be used, and
                were very useful for visualizing things like filter functions that changed over
                time.
            </p>
